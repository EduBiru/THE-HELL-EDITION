...
function createAccordionGroup(group) {
  ...
  // REMOVIDO: Controles foram para a barra de ações
    ...
    <table class="transaction-table">
      ...
      <tbody>
        ${group.transactions.map((t, index) => `
          <tr data-transaction-index="${index}">
  ...
  content.querySelectorAll('.edit-transaction-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const transactionIndex = e.target.closest('tr').dataset.transactionIndex;
      const transaction = group.transactions[transactionIndex];
      // ABRIR UM MODAL DE EDIÇÃO DE TRANSAÇÃO (a ser criado)
      showNotification(`Editar transação: ${transaction.hist}`, 'warning');
    });
  });
    ...
    options: {
      ...
      plugins: {
        ...
        zoom: {
          ...
          zoom: { 
            wheel: { enabled: true, modifierKey: 'ctrl' }, // Requer CTRL para zoom
            pinch: { enabled: true }, 
            mode: 'x' 
          }
        }
...
function renderActivityCalendar() {
  ...
  if (dataToUse.length === 0) {
    grid.innerHTML = '<div class="empty-state" style="grid-column: 1 / -1;">Sem dados de débito para exibir</div>';
    return;
  }
  ...
  // Determina o ano a ser exibido (o mais recente dos dados filtrados)
  const latestYear = Math.max(...dataToUse.map(t => t.date.getFullYear()));
  const startDate = new Date(latestYear, 0, 1);
  const endDate = new Date(latestYear, 11, 31);

  // Preenche os dias vazios no início do ano para alinhar com o domingo
  const dayOfWeek = startDate.getDay();
  for (let i = 0; i < dayOfWeek; i++) {
    const cell = document.createElement('div');
    cell.className = 'heatmap-cell-placeholder'; // Célula vazia sem interação
    grid.appendChild(cell);
  }

  // Cria uma célula para cada dia do ano
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    const dateStr = d.toISOString().split('T')[0];
    const value = dailyDebits.get(dateStr) || 0;

    const cell = document.createElement('div');
    cell.className = 'heatmap-cell';

    // Define o nível de cor
    let level = 0;
    if (value > 0) {
      const percentage = value / maxDebit;
    ...
    // Adiciona tooltip e evento de clique
    const formattedDate = d.toLocaleDateString('pt-BR', { timeZone: 'UTC' });
    cell.title = `${formattedDate}: ${formatCurrency(value)}`;
    cell.addEventListener('click', () => {
      document.getElementById('mainSearch').value = formattedDate;
      FILTERS.search = formattedDate;
      saveFilters();
      applyFiltersAndSort();
      renderAccordion();
      showNotification(`Filtrando por ${formattedDate}`, 'success');
    });
...
function setupEventListeners() {
  ...
  document.getElementById('groupSearch').addEventListener('input', debounce(handleGroupSearchInput, 200)); // NOVO
  ...
  document.getElementById('move-selected-btn').addEventListener('click', handleMoveSelected);
  document.getElementById('ignore-selected-btn').addEventListener('click', handleIgnoreSelected);
  document.getElementById('print-selected-btn').addEventListener('click', handlePrintSelected);
...
function handleFileUpload(e) {
  ...
  reader.onload = e => {
    try {
      ...
      setTimeout(() => uploadWrapper.classList.remove('success'), 2000);
...
function handlePrintScreen() {
  ...
  document.querySelectorAll('.modal-overlay, .header-controls').foram(el => el.style.visibility = 'hidden');
...
function handleGroupSearchInput(e) {
  FILTERS.groupSearch = e.target.value;
  // Não precisa salvar, é um filtro volátil
  
  const search = canonicalKey(FILTERS.groupSearch);
  const groups = document.querySelectorAll('#accordion-container > div');
  
  groups.forEach(groupEl => {
    const groupName = groupEl.dataset.groupElName;
    const isMatch = groupName && canonicalKey(groupName).includes(search);
    groupEl.classList.toggle('hidden', !isMatch);
  });
}
...
function clearFilters() {
  ...
  document.getElementById('groupSearch').value = ''; // Limpa busca local
...
function handlePrintGroup(group) {
  ...
  const tableRows = group.transactions
    .sort((a, b) => a.date - b.date) // Ordena por data para a impressão
    .map(t => `
      <tr>
        <td>${formatDate(t.date)}</td>
        <td>${t.hist}</td>
  ...
  const printContent = `
    ...
    <head>
      ...
      <style>
        ...
        .summary { margin-top: 2rem; padding-top: 1rem; border-top: 2px solid #333; }
        .summary-item { display: flex; justify-content: space-between; font-size: 1.1rem; padding: 0.5rem 0; }
        .summary-item strong { font-weight: 600; }
        @media print {
          body { margin: 1rem; }
          h1 { font-size: 1.2rem; }
          p, table, .summary-item { font-size: 10pt; }
        }
      </style>
    </head>
    <body>
      <h1>Relatório de Transações</h1>
      <p>Grupo: <strong>${group.name}</strong></p>
      ...
      <table>
        <thead>
          <tr>
            <th>Data</th>
            <th>Histórico</th>
            <th class="value">Crédito</th>
            <th class="value">Débito</th>
          </tr>
        </thead>
        <tbody>
          ${tableRows}
      ...
      <script>
        // Aciona a impressão automaticamente e fecha a janela depois
...
function handlePrintSelected() {
  ...
  handlePrintGroup(groupsToPrint);
}
...
function loadFilters() {
  try {
    ...
    if (storedFilters) {
      ...
      FILTERS.groupUnknown = parsed.groupUnknown || false;
      FILTERS.showIgnored = parsed.showIgnored !== undefined ? parsed.showIgnored : true;
      CURRENT_YEARS = parsed.years || [];
      CURRENT_SORT = parsed.sort || { key: 'name', order: 'asc' };
    }
    
    // Aplica filtros carregados na UI
    document.getElementById('mainSearch').value = FILTERS.search;
    document.getElementById('groupUnknown').checked = FILTERS.groupUnknown;
    document.getElementById('showIgnored').checked = FILTERS.showIgnored;
    // (renderYearFilters e handleSortClick irão aplicar os estados de ano/sorteio na UI)
...
// --- Inicia o aplicativo ---
// **** A CORREÇÃO MAIS IMPORTANTE (PLANO F) ****
// Voltamos ao DOMContentLoaded.
// Isso GARANTE que o 'main' vai rodar e os botões vão funcionar.
// A verificação do heatmap agora é feita DENTRO do renderActivityCalendar.
